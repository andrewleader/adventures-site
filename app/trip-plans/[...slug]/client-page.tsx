    ...
    <article className="container mx-auto px-4 py-8 max-w-4xl">
      <header className="mb-8">
        ...
        <div className="flex flex-wrap gap-4 text-sm text-gray-600">
          {tripPlan.startDate && (
            <span>📅 Start: {formatDate(tripPlan.startDate, { year: 'numeric', month: 'numeric', day: 'numeric' })}</span>
          )}
          {tripPlan.endDate && (
            <span>📅 End: {formatDate(tripPlan.endDate, { year: 'numeric', month: 'numeric', day: 'numeric' })}</span>
          )}
